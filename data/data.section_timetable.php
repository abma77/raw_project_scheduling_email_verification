<?php
    ...
    if(isset($_POST["submit"])){
        if(!empty($_POST["submit"])){
            ...
            $retrieve_section_schedule = $conn->prepare("
                SELECT * 
                FROM schedule
                LEFT JOIN subject
        ...
        <tr class="text-center">
            ...
            <?php
                foreach($weeks as $week){
                    echo "
                        <th style='font-size: 12px;'class='bg-primary'>".$week."</th>
                    ";
                }
            ?>
        </tr>
                                            ...
                                            echo "
                                                <div class='item row-span-".$val['schedule_rowspan']."'>
                                                    <p style='font-size: 10px;' class='text-center sched' data-id='".$val['schedule_id']."' data-bs-toggle='modal' data-bs-target='#staticBackdrop'>
                                                        ...
                                                        <br>
                                                        ".$val["subject_name"]." - ".$val["schedule_room"]."
                                                    </p>
                                                </div>
                                            ";
                                        }
...
<?php 
    $retrieve_section_schedule = null;
    $conn = null;
?>