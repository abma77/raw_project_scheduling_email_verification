<?php
    ...
    if(isset($_POST["submit"])){
        if(!empty($_POST["submit"])){
            ...
            $schedule_start_time = date("H:i", strtotime($_POST["start_time_hour"].$_POST["start_time_minute"]));
            $schedule_end_time = date("H:i", strtotime($_POST["end_time_hour"].$_POST["end_time_minute"]));
            ...
            if($section_row = $check_section_time->fetch()){
                ...
                $insert_section_schedule = $conn->prepare("
                    ...
                    (schedule_semester, schedule_school_year, schedule_room, schedule_section, schedule_week_day, schedule_teacher, schedule_subject, schedule_start_time, schedule_end_time, schedule_rowspan)
                    VALUES
                    (?,?,?,?,?,?,?,?,?,?,?)
                ");
